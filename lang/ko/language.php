...
"Sets the number of comments per page" => "페이지 당 댓글의 수를 설정 (기본 = 10)",
"comments" => "댓글",
"Display order" => "기본 순서",
...
"Allow forbidden items in search results" => "검색 결과 내부에 금지된 항목 허용",
"Object filter" => "개체 필터",
...
"Adds a link at the bottom if set, otherwise adds a link at the top" => "설정된 경우 하단에 링크 추가, 그 외는 상단에 링크 추가",
"Shadow layer" => "음영 레이어",
...
"As much as possible, attempt to not display the email address, showing the real name or the truncated email address instead." => "이메일 대신 실명 혹은 축약된 이메일 주소를 표시하여, 이메일을 최대한 숨기려는 시도를 하게 됩니다.",
...
"Menu icons" => "메뉴 아이콘",
"Enable icons to be defined for menu items" => "메뉴 항목에 대한 아이콘 정의 허용",
...
"Meta robots" => "메타 로봇",
"Specify how web robots should index this site. Valid values include: INDEX or NOINDEX, and FOLLOW or NOFOLLOW" => "웹봇이 귀하의 사이트를 어떻게 색인해야하는지를 지정합니다. 유효한 값들은 다음을 포함합니다: INDEX 혹은 NOINDEX, 그리고 FOLLOW 혹은 NOFOLLOW",
"Revisit after" => "다음 이 후 재 방문",
"How often (in days) web robots should visit this site" => "귀하의 사이트를 웹봇이 얼마나 자주 (일 단위) 방문해야하는지를 지정합니다",
...
"Visibility of horizontal zone for modules such as site logo, log-in form, etc. (page header)" => "로고, 로그인 등의 (헤더)와 같은 모듈을 유지할 영역의 표시 여부",
"Topbar module zone" => "Topbar 모듈 영역",
"Visibility of horizontal  zone for modules such as menu (navbar), search form, full-width content, etc" => "메인 가로 메뉴, 검색 형식, 페이지 너비 컨텐츠, 기타 등등과 같은 모듈들의 영역의 표시 여부",
"topbar" => "TopBar",
"Page top module zone" => "페이지 상단 모듈 영역",
"Visibility of horizontal zone above main page area for modules such as breadcrumbs, banners, share icons, etc" => "공유와 같은 모듈들을 유지하기 위한 영역의 표시 여부",
"Bottom module zone" => "하단 모듈 영역",
"Visibility of horizontal zone for modules such as \"powered by\" and \"rss list\" (page footer)" => "\"powered by\" 및 \"rss list\" 와 같은 모듈을 유지할 영역 (바닥글)의 표시 여부",
"Page bottom module zone" => "페이지 하단 모듈 영역",
"Visibility of horizontal zone below the main page area for modules such as banners or other information" => "각 페이지의 하단에 모듈을 유지할 영역의 표시 여부",
"Module file" => "모듈 파일",
...
"Number of emails to send in each batch" => "각 일괄작업 내에 보낼 이메일의 양",
...
"file" => "파일",
"Wiki buttons" => "위키 버튼",
"The location of the row of Edit, Source, Rename, History, More buttons, etc." => "페이지 설명, 아이콘, 백링크, ..",
"Top " => "상위",
"Bottom" => "아래쪽",
"Neither" => "둘 다 아님",
"A page can be listed multiple times in a structure" => "한 페이지는 한 구조체에서 여러번 발생할 수 있습니다",
"Display 'first' and 'last' links on pages" => "설정된 경우, 페이지에 '처음'와 '마지막'을 표시할 것입니다",
...
"Wiki page containing the profile for creating order items (page name must not have spaces)" => "주문 항목 만들기 위한 프로필이 있는 위키 페이지 (페이지명에는 스페이스가 없어야 함)",
...
"Use this security service provided by Google instead of the default CAPTCHA" => "특화된 CAPTCHA 서비스인 ReCaptcha 를 기본 CAPTCHA 대신 사용",
...
"The search engine used to index the content of this Tiki site. Some engines are more suitable for larger sites, but require additional software on the server." => "티키의 컨텐츠를 색인화하기 위해 사용되는 검색 엔진. 어떤 검색 엔진은 보다 큰 사이트에 더 적당할 수있으나, 서버 상에 추가 소프트웨어를 요구하기도 함.",
...
"Highlight the result snippet based on the search query to improve user experience" => "검색 질의에 기반한 결과 조각을 강조. 이 선택사항을 활성화하면 성능에 영향을 미치게 될 것입니다만 사용자 경험을 향상할 것입니다.",
...
"Maximum size of result set to consider" => "고려할 결과 세트의 최대 크기. 기본 1000, 0 = 무제한",
...
"Maximum number of terms to be generated. This value may need to be increased in the case of \"Terms per query limit is reached\" errors, especially with wildcard, range and fuzzy searches" => "생성될 조건의 최대 수. \" 질의 당 조건 제한에 도달했습니다\" 라는 오류를 얻게 되는 경우, 특별히 와일드카드, 범위 및 퍼지 검색의 경우, 이 값의 증가를 시도해 보십시오",
...
"Search formatters whose output will be cached" => "출력을 캐시할 검색 포맷터",
...
"Enter the comma-separated IDs of trackers to sync user preferences from." => "사용자 기본설정을 동기화할 트래커들의 쉼표로 구분된 ID들을 입력하십시오",
...
"Use jQuery autocomplete text input to prevent out-of-memory errors and performance issues when the user list is very large" => "사용자 목록이 매우 큰 경우 jQuery 자동완성 텍스트 입력 상자를 사용하여 메모리 부족과 성능 문제를 예방",
"users" => "사용자",
...
"Token" => "토큰",
"Maximum size of each attachment" => "각 첨부파일에 대한 최대 크기",
"Unlimited" => "무제한",
"Last editor" => "최종 편집자",
...
"Combine site features to create a workspace experience for work groups" => "티키 내의 여러 기능들을 묶어서 작업그룹에 대하여 작업공간 경험을 형성",
...
"Jabber" => "재버",
"Chat using the Jabber instant messaging client." => "Jabber 를 사용한 채팅",
...
"Scroll" => "스크롤",
"Create a section of text that is scrolled up and down with JavaScript." => "위나 아래로 스크롤하는 움직이는 텍스트 표시",
"SourceForge" => "소스포지",
...
"Incorrect parameter: " => "틀린 매개 변수:",
"Subscript" => "아래 첨자",
"Create subscript text." => "텍스트에 아래 첨자 폰트를 적용",
...
"Superscript" => "위 첨자",
"Create superscript text." => "위 첨자 폰트를 텍스트에 적용",
"Wiki Page to redirect the user after his vote" => "투표 후 사용자가 보내지는 위키 페이지",
"Create a set of tabs to contain page content." => "탭 내부에 컨텐츠 배열",
...
"Tracker If" => "트래커 If",
"Display content based on the results of a tracker field test." => "트래커 필드 테스트의 결과에 기반한 내용 표기",
"Ignore" => "무시",
...
"Create a button to enable users to prefill tracker fields." => "트래커 필드를 사전작성할 버튼 만들기",
...
"Transclude" => "트랜스클루드",
"Include the content of another page, with specified changes." => "특정 변경내역을 가진 다른 페이지의 컨텐츠 포함",
...
"Create tabs to show alternate versions of content, such as earlier and updated information." => "컨텐츠의 다른 버전들을 표시하는 탭 만들기",
...
"Show the locations (on wiki pages) of links to pages not yet created." => "아직 만들어지지 않은 페이지로의 링크의 위치 표시",
"Split By" => "다음으로 분할됨",
...
"Retrieve and include a Zotero reference in the page" => "페이지에 조테로 참조를 검색하고 포함",
"Reference Key" => "참조 키",
"Reference Tag" => "참조 태그",
...
"Use double quotes ( <kbd>\" \"</kbd> ) around a phrase to find terms in the exact order, next to each other" => "정확한 순서대로 조건을 찾기 위하여 나란히 놓여진 문구의 주위에 따옴표 사용",
"Example: <kbd>\"Alex Bell\"</kbd> will not find <strong>Bell Alex</strong> or <strong>Alex G. Bell</strong> but <strong>Alex Bells</strong>. " => "예: <kbd>\"Alex Bell\"</kbd> 은 <strong>Bell Alex</strong> 혹은 <strong>Alex G. Bell</strong> 을 찾지 못하지만, <strong>Alex Bells</strong> 은 찾을 것입니다. ",
"Indexed" => "들여쓰기",
"Queue size" => "대기 크기",
"version" => "버전",
"Index" => "색인",
