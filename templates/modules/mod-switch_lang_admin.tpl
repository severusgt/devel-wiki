{* $Id: mod-switch_lang_admin.tpl 66534 2018-05-28 22:32:14Z xorti $ *}
{if $tiki_p_admin == 'y'}
{strip}
	{tikimodule error=$module_params.error title=$tpl_module_title name="switch_lang_admin" flip=$module_params.flip decorations=$module_params.decorations nobox=$module_params.nobox notitle=$module_params.notitle}
		...
		{elseif $mode eq 'words' || $mode eq 'abrv'}
			<ul>
				{section name=ix loop=$languages}
					<li>
						{if $mode eq 'words'}
							<a title="{$languages[ix].name|escape}" class="linkmodule {$languages[ix].class}" href="tiki-switch_lang.php?languageAdmin={$languages[ix].value|escape}">
		...
		{else}{* do menu as before is not flags or words *}
			<form method="get" action="tiki-switch_lang.php" target="_self">
				<select name="languageAdmin" size="1" onchange="this.form.submit();" class="form-control">
					{section name=ix loop=$languages}
